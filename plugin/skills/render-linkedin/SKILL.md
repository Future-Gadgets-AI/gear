...
5. Apply linkedin.md's rule on optional platform tag markers (rule 4): derive each one only from the spec's `tags` field, converting kebab-case to a `#`-prefixed CamelCase token (e.g. `dark-factory` → `#DarkFactory`) — never invent a tag beyond what `tags` lists; if `tags` yields more than the contract's cap, keep document order and truncate. They sit after the value block, before the closing line.
